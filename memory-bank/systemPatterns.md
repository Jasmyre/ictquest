...
- URL-transparent route groups give each app area its own layout and access rules:
  ...
  - `(admin)/` — guarded shell; hosts `/admin`. The layout wraps children in `<Suspense fallback={<AdminShell …>…}><AdminGuard/></Suspense>` (the fallback is the static admin shell, not `null` — a `null` fallback committed a blank frame on soft navigations while the gate resolved, caught by the `instant()` guard), where `AdminGuard` (`src/components/admin-guard.tsx`) awaits `connection()` + `auth()` and redirects signed-out callers to `/auth` and suspended/non-ADMIN callers to `/`. Inside the guard, the same `MainSidebar` shell renders (`AdminShell`/`AdminShellAsync`, `groupLabel="Admin"`). The role check re-reads memberships plus the suspension flag per request with session fallback, per the session-freshness rule; signed-out visitors never reach the guard because the proxy redirects them off `/admin` first.
